...
"""
from PyQt5.QtWidgets import (QDialog,QScrollArea,QWidget,QGridLayout,QListWidgetItem,
                             QLabel,QSpacerItem,QSizePolicy,QHBoxLayout,QVBoxLayout,
                             QPushButton,QApplication)
from PyQt5.QtCore import QRect,QTimer,QReadWriteLock
...
class pairDataDlg(QDialog):
    def __init__(self,dict_types,dataset_cont,lock,parent=None):
        ...
        try:
            ...
            for name, tmp in dataset_cont:
                for key in  list(dict_types.keys()):
        ...
        data_label = list(dict_types.keys())
        ...
        type_list = list(self.dictListWidgetType.keys())
        ...
        self.pushButtonSort.clicked.connect(self.automaticSort)
        self.pushButtonContinue.clicked.connect(self.continuePressed)
        pushButtonAdd.clicked.connect(self.addPairedAndClear)
        self.pushButtonDivide.clicked.connect(self.dividePerType)
    ...
    def dividePerType(self):
        for k in range(self.list_all_data.count()):
            ...
            for key in list(self.dictListWidgetType.keys()):
...
#            return
        first_list = list(self.dictListWidgetType.keys())[0]
...
#            if expected_el != 
        ...
        for key in list(self.dictListWidgetType.keys())[1:]:
            key_text = []
    ...
    def checkLists(self):
        counts = np.zeros(len(list(self.dictListWidgetType.keys())),dtype=int)
        i = 0
        for key in list(self.dictListWidgetType.keys()):
...
#            self.pushButtonSort.setEnabled(False)
    ...
    def setContinueStatus(self):
        counts = np.zeros(len(list(self.dictListWidgetType.keys())),dtype=int)
        i = 0
        for label in list(self.dictListWidgetType.keys()):
    ...
    def create_dict_pairs(self):
        ...
        pairMatrix['Subject_num'] = list(range(num_el))
        return pairMatrix
...
def main():
    ...
    datadict = np.load('/Users/Matte/Desktop/Paper marta/data/Sleep phz/baseline/PWS_22.phz')
#    datadict2 = np.load('C:\Users\ebalzani\Desktop\Data\Sleep\\workspace_2017-5-15T14_16.phz')
    print(list(datadict.keys()))
    th  = len(list(datadict.keys()))//2
    i = 0
