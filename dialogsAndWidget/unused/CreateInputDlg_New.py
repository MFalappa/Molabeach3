...
class inputDialog(QDialog):
    ...
    def __init__(self,DataName,comboBox=[],TimeSpinBoxLabel=[],DoubleSpinBox=[],
                ...
                ActivityList=[],comboValueLists = None,parent=None):
        super(inputDialog,self).__init__(parent)
        
        self.comboValues = []
        if comboValueLists is None:
            self.comboValueLists = [[]]*len(comboBox)
        else:
            self.comboValueLists = comboValueLists
...
if __name__ == "__main__":       
    ...
    form = inputDialog(None,comboBox,timeSpinBox,doubleSpinBox,LineEdit,SpinBox,True,3,
                       ActivityList=['Ciao','Gatto','Ciccio'],
                        comboValueLists=[[1000,'finedelmondo'],np.arange(0,4,0.1)])
    ...
    value = form.comboValues
    print(value)
    print(form.ComboBox[0].count(),form.ComboBox[1].count())
